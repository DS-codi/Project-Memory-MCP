...
## Non-Negotiable Rules (Enforced, Not Optional)
...
### 2. Update Steps in Real Time — This Is Mandatory

**Step status MUST reflect the live state of work at all times.** This is not optional hygiene — it is the primary observability mechanism for the entire system. Skipping or deferring step updates makes the plan invisible to the user and to other agents.

**The exact update protocol:**

1. **Before touching any file or running any command for a step** → mark it `active`
2. **Immediately after completing a step** → mark it `done` with specific notes (files changed, outcome)
3. **As soon as a step becomes blocked** → mark it `blocked` with a full description of what failed and why

```json
// STEP 1: The very first thing before starting work
{ "action": "update", "step_index": 3, "status": "active", "notes": "Starting implementation" }

// STEP 2: Immediately after finishing, before moving to the next step
{ "action": "update", "step_index": 3, "status": "done", "notes": "Implemented X in src/foo.ts, updated tests" }

// STEP 3: When blocked — update immediately, do not continue
{ "action": "update", "step_index": 3, "status": "blocked", "notes": "Cannot proceed: missing dependency Y, build fails with error Z" }
```

**If multiple steps are completed within a single agent session, each one MUST be individually marked `active` then `done` as it is completed — not batch-updated at the end.**
...
### ❌ Don't Batch Step Updates at Session End

```json
// BAD - Agent completes 4 steps, then batch-updates all at the end
// This means the plan showed 0% progress for the entire session.
// The user and hub had no visibility. If the session was interrupted,
// all steps remain in "pending" even though work was done.
...
// GOOD - Each step is updated as it happens:
// Before step 0: mark active → do work → mark done
// Before step 1: mark active → do work → mark done
// ... and so on. Plan reflects real state at every point.
```

### ❌ Don't Write Vague Step Notes

```json
// BAD - Notes tell nothing about what was actually done
{ "action": "update", "step_index": 2, "status": "done", "notes": "Done" }
{ "action": "update", "step_index": 2, "status": "done", "notes": "Completed" }

// GOOD - Notes are specific and useful for recovery, review, and handoff
{ "action": "update", "step_index": 2, "status": "done",
  "notes": "Added updateBuildGatePanel() to client-helpers.ts; wired /api/plans/:id/build-scripts; zero TS errors" }
```
...
- **Update immediately — not at the end:** Mark steps `active` before touching a file, `done` the moment the step is finished. Never defer updates to end-of-session.
- **Document blockers fully:** When marking blocked, include what command failed, what the error was, and what is needed to unblock
- **Log your work:** Store execution logs for future reference
- **Specific notes required:** Notes must name the files changed and the outcome — not just "done"
