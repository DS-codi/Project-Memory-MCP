...
/// Configuration parameters used to construct a [`BackoffState`].
///
/// This mirrors the `ReconnectSection` from the supervisor's config module, but
/// is kept local to supervisor-iced so there is no cross-crate dependency.
#[derive(Debug, Clone)]
pub struct BackoffConfig {
    /// Starting delay in milliseconds.
    pub initial_delay_ms: u64,
    /// Upper cap on computed delays in milliseconds.
    pub max_delay_ms: u64,
    /// Exponential multiplier applied to the base after each attempt.
    pub multiplier: f64,
    /// Fraction of the current base delay added as random jitter (e.g. `0.2` = 20 %).
...
impl BackoffState {
    ...
    /// Construct from a [`BackoffConfig`] block.
...
mod tests {
    ...
    fn reset_restores_initial_state() {
        ...
        assert_eq!(
            b.next_delay_ms(),
            500,
            "after reset first delay should be initial_ms again"
        );
    }

    #[test]
    fn from_config_uses_provided_jitter() {
