...
impl ServiceStateMachine {
    ...
    pub fn new(reconnect_config: &ReconnectSection, service_id: &str, restart_policy: RestartPolicy) -> Self {
        let backoff_config = BackoffConfig {
            initial_delay_ms: reconnect_config.initial_delay_ms,
            max_delay_ms: reconnect_config.max_delay_ms,
            multiplier: reconnect_config.multiplier,
            jitter_ratio: reconnect_config.jitter_ratio,
        };
        Self {
            state: ConnectionState::Disconnected,
            backoff: BackoffState::from_config(&backoff_config),
