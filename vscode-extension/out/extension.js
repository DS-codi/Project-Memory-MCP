...
var AgentWatcher = class {
  ...
  start() {
    ...
    this.watcher.on("change", async (filePath) => {
      ...
      if (this.autoDeploy) {
        vscode2.window.showInformationMessage(`Deploying updated agent: ${agentName}`);
      } else {
    ...
    this.watcher.on("add", (filePath) => {
      const agentName = path2.basename(filePath, ".agent.md");
      vscode2.window.showInformationMessage(`New agent template detected: ${agentName}`);
    });
...
var CopilotFileWatcher = class {
  ...
  async handleFileEvent(type, filePath, action) {
    ...
    if (action === "add") {
      vscode3.window.showInformationMessage(`New ${label.toLowerCase()} detected: ${fileName}`);
      return;
    }
    if (this.config.autoDeploy) {
      vscode3.window.showInformationMessage(`Auto-deploying updated ${label.toLowerCase()}: ${fileName}`);
      this.triggerDeploy(type);
    } else {
      const deployAction = await vscode3.window.showInformationMessage(
...
var ServerManager = class {
  ...
  async start() {
    ...
    if (existingServer) {
      ...
      vscode5.window.showInformationMessage("Connected to existing Project Memory server");
      return true;
    }
    const serverDir = this.getServerDirectory();
    if (!serverDir) {
      vscode5.window.showErrorMessage("Could not find dashboard server directory");
      return false;
    }
...
function activate(context) {
  ...
  const config = vscode8.workspace.getConfiguration("projectMemory");
  ...
  if (autoDeployOnWorkspaceOpen && vscode8.workspace.workspaceFolders?.[0]) {
    ...
    serverManager.start().then(async (success) => {
      if (success) {
        if (serverManager.isExternalServer) {
          vscode8.window.showInformationMessage("Connected to existing Project Memory server");
        } else {
          vscode8.window.showInformationMessage("Project Memory API server started");
        }
      } else {
  ...
  context.subscriptions.push(
    vscode8.commands.registerCommand("projectMemory.showDashboard", () => {
      vscode8.commands.executeCommand("workbench.view.extension.projectMemory");
    }),
    vscode8.commands.registerCommand("projectMemory.openDashboardPanel", async (url) => {
      if (!serverManager.isRunning) {
        const startServer = await vscode8.window.showWarningMessage(
          "Project Memory server is not running. Start it first?",
          "Start Server",
          "Cancel"
        );
        if (startServer !== "Start Server") return;
        const success = await vscode8.window.withProgress({
        ...
        if (!success) {
          vscode8.window.showErrorMessage("Failed to start server. Check logs for details.");
          serverManager.showLogs();
          return;
        }
      }
      if (!serverManager.isFrontendRunning) {
        ...
        if (!success) {
          vscode8.window.showErrorMessage("Failed to start dashboard frontend. Check server logs.");
          serverManager.showLogs();
          return;
        }
      }
    ...
    vscode8.commands.registerCommand("projectMemory.toggleServer", async () => {
      if (serverManager.isRunning) {
        await serverManager.stopFrontend();
        await serverManager.stop();
        vscode8.window.showInformationMessage("Project Memory server stopped");
      } else {
        const success = await serverManager.start();
        if (success) {
          vscode8.window.showInformationMessage("Project Memory server started");
        } else {
          vscode8.window.showErrorMessage("Failed to start Project Memory server");
        }
      }
    }),
    vscode8.commands.registerCommand("projectMemory.startServer", async () => {
      if (serverManager.isRunning) {
        vscode8.window.showInformationMessage("Server is already running");
        return;
      }
      const success = await serverManager.start();
      if (success) {
    ...
    vscode8.commands.registerCommand("projectMemory.stopServer", async () => {
      await serverManager.stopFrontend();
      await serverManager.stop();
      vscode8.window.showInformationMessage("Project Memory server stopped");
    }),
    vscode8.commands.registerCommand("projectMemory.restartServer", async () => {
      vscode8.window.showInformationMessage("Restarting Project Memory server...");
      await serverManager.stopFrontend();
      const success = await serverManager.restart();
      if (success) {
        vscode8.window.showInformationMessage("Project Memory server restarted");
      } else {
        vscode8.window.showErrorMessage("Failed to restart server");
      }
    }),
    vscode8.commands.registerCommand("projectMemory.showServerLogs", () => {
      serverManager.showLogs();
    }),
    vscode8.commands.registerCommand("projectMemory.openSettings", () => {
      vscode8.commands.executeCommand("workbench.action.openSettings", "@ext:project-memory.project-memory-dashboard");
    }),
    vscode8.commands.registerCommand("projectMemory.createPlan", async () => {
      const workspaceFolders = vscode8.workspace.workspaceFolders;
      if (!workspaceFolders) {
        vscode8.window.showErrorMessage("No workspace folder open");
        return;
      }
      const approach = await vscode8.window.showQuickPick(
        [
      ...
      if (approach.value === "brainstorm") {
        const initialPrompt = await vscode8.window.showInputBox({
        ...
        try {
          ...
          if (result === "Copy Prompt") {
            await vscode8.env.clipboard.writeText(`@brainstorm ${initialPrompt}`);
            vscode8.window.showInformationMessage("Prompt copied to clipboard");
          }
        }
        return;
      }
      ...
      if (!category) return;
      const priority = await vscode8.window.showQuickPick(
        [
      ...
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          vscode8.window.showInformationMessage(`Plan created: ${title}`, "Open Dashboard").then((selection) => {
          ...
          vscode8.window.showErrorMessage(`Failed to create plan: ${error}`);
        }
      } catch (error) {
        vscode8.window.showErrorMessage(`Failed to create plan: ${error}`);
      }
    }),
    vscode8.commands.registerCommand("projectMemory.deployAgents", async () => {
      const workspaceFolders = vscode8.workspace.workspaceFolders;
      if (!workspaceFolders) {
        vscode8.window.showErrorMessage("No workspace folder open");
        return;
      ...
      try {
        ...
        vscode8.window.showErrorMessage(`Failed to deploy agents: ${error}`);
      }
    }),
    vscode8.commands.registerCommand("projectMemory.deployPrompts", async () => {
      const workspaceFolders = vscode8.workspace.workspaceFolders;
      if (!workspaceFolders) {
        vscode8.window.showErrorMessage("No workspace folder open");
        return;
    ...
    vscode8.commands.registerCommand("projectMemory.deployCopilotConfig", async () => {
      ...
      const confirm = await vscode8.window.showQuickPick(["Yes", "No"], {
      ...
      if (confirm === "Yes") {
        ...
        vscode8.window.showInformationMessage("Deploying all Copilot configuration...");
      }
    }),
    vscode8.commands.registerCommand("projectMemory.deployDefaults", async () => {
      const workspaceFolders = vscode8.workspace.workspaceFolders;
      if (!workspaceFolders) {
        vscode8.window.showErrorMessage("No workspace folder open");
        return;
    ...
    vscode8.commands.registerCommand("projectMemory.updateDefaults", async () => {
      const workspaceFolders = vscode8.workspace.workspaceFolders;
      if (!workspaceFolders) {
        vscode8.window.showErrorMessage("No workspace folder open");
        return;
      ...
      if (result.updated.length > 0 || result.added.length > 0) {
        ...
        vscode8.window.showInformationMessage("All files are up to date");
      }
    }),
    vscode8.commands.registerCommand("projectMemory.openAgentFile", async () => {
      const config2 = vscode8.workspace.getConfiguration("projectMemory");
      const agentsRoot2 = config2.get("agentsRoot") || getDefaultAgentsRoot();
      if (!agentsRoot2) {
        vscode8.window.showErrorMessage("Agents root not configured");
        return;
      }
      const fs2 = require("fs");
      const path6 = require("path");
      try {
        ...
        const selected = await vscode8.window.showQuickPick(files, {
        ...
        vscode8.window.showErrorMessage(`Failed to list agent files: ${error}`);
      }
    }),
    vscode8.commands.registerCommand("projectMemory.openPromptFile", async () => {
      const config2 = vscode8.workspace.getConfiguration("projectMemory");
      ...
      try {
        ...
        vscode8.window.showErrorMessage(`Failed to list prompt files: ${error}`);
      }
    }),
    vscode8.commands.registerCommand("projectMemory.showCopilotStatus", () => {
      dashboardProvider.postMessage({ type: "showCopilotStatus" });
      vscode8.commands.executeCommand("workbench.view.extension.projectMemory");
    }),
    vscode8.commands.registerCommand("projectMemory.refreshData", () => {
      dashboardProvider.postMessage({ type: "refresh" });
    }),
    vscode8.commands.registerCommand("projectMemory.openFile", async (filePath, line) => {
      try {
        ...
        if (line !== void 0) {
          const position = new vscode8.Position(line - 1, 0);
          editor.selection = new vscode8.Selection(position, position);
          editor.revealRange(new vscode8.Range(position, position), vscode8.TextEditorRevealType.InCenter);
        }
      } catch (error) {
        vscode8.window.showErrorMessage(`Failed to open file: ${filePath}`);
      }
    }),
    // Add to Plan command - context menu for files
    vscode8.commands.registerCommand("projectMemory.addToPlan", async (uri) => {
      let filePath;
      let selectedText;
      let lineNumber;
      if (uri) {
      ...
      if (!filePath) {
        vscode8.window.showErrorMessage("No file selected");
        return;
      }
      const workspaceFolders = vscode8.workspace.workspaceFolders;
      if (!workspaceFolders) {
      ...
      const stepTask = await vscode8.window.showInputBox({
      ...
      const phase = await vscode8.window.showQuickPick(
...
\`\`\`` : void 0
      ...
      vscode8.window.showInformationMessage(`Added step to plan: "${stepTask}"`);
    })
  );
  if (agentsRoot) {
  ...
  context.subscriptions.push(
    vscode8.workspace.onDidChangeConfiguration((e) => {
      if (e.affectsConfiguration("projectMemory")) {
        const newConfig = vscode8.workspace.getConfiguration("projectMemory");
...
function getDefaultDataRoot() {
  const workspaceFolders = vscode8.workspace.workspaceFolders;
  if (workspaceFolders) {
    return vscode8.Uri.joinPath(workspaceFolders[0].uri, "data").fsPath;
  }
  return "";
}
function getDefaultAgentsRoot() {
  const workspaceFolders = vscode8.workspace.workspaceFolders;
  if (workspaceFolders) {
    return vscode8.Uri.joinPath(workspaceFolders[0].uri, "agents").fsPath;
  }
  return "";
}
