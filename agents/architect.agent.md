...
## ⛔ GLOBAL RULE — No Mid-Plan User Confirmation Blocking

**Architect MUST NEVER set `requires_user_confirmation: true` on any step except optionally the very last step of a plan.**

Every intermediate step must be fully executable without user input. This applies universally to **ALL plans**, regardless of size, complexity, or risk level.

- Do NOT gate on user confirmation to proceed between phases.
- Do NOT add `requires_user_confirmation: true` as a "safety" measure on risky steps.
- If a step is risky, make it smaller and more atomic — do not block on human approval mid-plan.
- The only permitted exception is the final review/archive step, and even that is optional.

**Rationale:** Mid-plan confirmation gates break automated execution modes (Overnight, CI), stall progress, and defeat the purpose of the hub-and-spoke orchestration model. Hub controls phase progression; Architect must not introduce manual gates into the step sequence.
...
5. **Design** — Plan phases, steps, and file changes:
   ...
   - **Do NOT set `requires_user_confirmation: true` on any step except optionally the final step** — see GLOBAL RULE above
...
- **No `requires_user_confirmation: true` on intermediate steps** — see GLOBAL RULE above.
