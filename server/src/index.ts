...
server.tool(
  ...
  'Consolidated workspace management tool. Actions: register (register a workspace directory), list (list all workspaces), info (get plans for a workspace), reindex (update codebase profile after changes), merge (merge a ghost/source workspace into a canonical target), scan_ghosts (scan for unregistered data-root directories), migrate (re-register workspace, find and merge all ghost/duplicate folders, recover plans, and clean up — use this when opening old workspaces), link (link/unlink parent-child workspace hierarchy), export_pending (export all unfinished steps from every plan to a .md file in the workspace), generate_focused_workspace (generate a focused workspace scope definition), list_focused_workspaces (list all focused workspace scope definitions), check_context_sync (compare .github/ agent and instruction files against DB — reports in_sync / local_only / db_only / content_mismatch per file so you can safely reconcile without data loss), import_context_file (explicitly preview or import an eligible DB-missing .github agent or instruction file; rejects PM-controlled, cull-policy, or metadata-invalid files and does not change watcher behavior), inject_cli_mcp (write or update .mcp.json at the workspace root so Claude Code agents running inside it automatically discover the CLI MCP server on port 3466; pass all_workspaces:true to inject into every registered workspace at once).',
  {
    ...
    all_workspaces: z.boolean().optional().describe('When true, inject_cli_mcp targets every registered workspace (ignores workspace_id/workspace_path)'),
    cli_mcp_port: z.number().int().positive().optional().describe('CLI MCP server port for inject_cli_mcp (default: PM_CLI_MCP_PORT env var or 3466)'),
...
server.tool(
  ...
  'Consolidated plan lifecycle management. Actions: list (list plans), get (get plan state), create (create new plan), update (modify plan steps), archive (archive completed plan), import (import existing plan file), find (find plan by ID), add_note (add note to plan), delete (delete plan), consolidate (consolidate steps), set_goals (set goals and success criteria), add_build_script (add build script), list_build_scripts (list build scripts), run_build_script (resolve build script for terminal execution), delete_build_script (delete build script), create_from_template (create plan from template), list_templates (list available templates), summon_approval (launch approval GUI for a plan step), summon_cleanup_approval (launch cleanup approval batch from plan-cleanup report payload), link_to_program (link plan to program), unlink_from_program (unlink plan from program), set_plan_dependencies (set plan dependencies), get_plan_dependencies (get plan dependencies and dependents), set_plan_priority (update plan priority), clone_plan (deep copy a plan), merge_plans (merge steps from multiple plans), pause_plan (pause plan at approval gate), resume_plan (resume paused plan), search (two-phase full-text search across programs/plans/phases/steps — first call returns a grouped summary with refine hints; add search_entity_type/plan_id/search_status/search_phase to get detailed results; auto-returns detail when only one match found).',
  {
    ...
    cleanup_report_path: z.string().optional().describe('Path to plan_cleanup_audit report JSON containing approval_gui_batch (for summon_cleanup_approval)'),
    cleanup_form_request: z.record(z.string(), z.unknown()).optional().describe('Cleanup approval form_request payload (for summon_cleanup_approval)'),
    cleanup_response_mapping: z.array(z.record(z.string(), z.unknown())).optional().describe('Cleanup approval response mapping payload (for summon_cleanup_approval)'),
    ...
    query: z.string().optional().describe('Full-text search query (for search action)'),
    search_entity_type: z.enum(['program', 'plan', 'phase', 'step']).optional().describe('Narrow search to this entity type (for search action; omit for grouped summary)'),
...
server.tool(
  'memory_agent',
  'Consolidated agent lifecycle and deployment tool. Actions: init (initialize agent session), complete (complete session), handoff (recommend next agent), validate (validate agent for current task), list (list available agents), get_instructions (get agent instructions), deploy (deploy agents to workspace), get_briefing (get mission briefing), get_lineage (get handoff history), categorize (store categorization decision on plan), deploy_for_task (deploy agent files + context bundle for a specific task). Skill discovery: list_skills (list all skills with metadata), get_skill (get full skill content by name), assign_skill (assign skill to workspace), unassign_skill (remove skill assignment), list_workspace_skills (get skills assigned to a workspace). Skill management: create_skill (create or update a skill — upsert by name; pass workspace_id to simultaneously assign to workspace), delete_skill (delete a skill by name). Instruction discovery: list_instructions (list all instruction files), get_instruction (get instruction content by filename), assign_instruction (assign instruction to workspace), unassign_instruction (remove instruction assignment), list_workspace_instructions (get instructions assigned to a workspace). Instruction management: create_instruction (create or update an instruction file — upsert by filename; pass workspace_id to simultaneously assign to workspace), delete_instruction (delete an instruction by filename).',
  {
    action: z.enum(['init', 'complete', 'handoff', 'validate', 'list', 'get_instructions', 'deploy', 'get_briefing', 'get_lineage', 'categorize', 'deploy_for_task', 'list_skills', 'get_skill', 'create_skill', 'delete_skill', 'assign_skill', 'unassign_skill', 'list_workspace_skills', 'list_instructions', 'get_instruction', 'create_instruction', 'delete_instruction', 'assign_instruction', 'unassign_instruction', 'list_workspace_instructions']).describe('The action to perform'),
    workspace_id: z.string().optional().describe('Workspace ID'),
    plan_id: z.string().optional().describe('Plan ID'),
    ...
    skill_name: z.string().optional().describe('Skill name (for get_skill, create_skill, delete_skill, assign_skill, unassign_skill)'),
    skill_category: z.string().optional().describe('Category filter (for list_skills) or skill category (for create_skill)'),
    skill_content: z.string().optional().describe('Full skill markdown content (for create_skill)'),
    skill_description: z.string().optional().describe('Short description of what this skill covers (for create_skill)'),
    skill_tags: z.array(z.string()).optional().describe('Tags for skill matching (for create_skill)'),
    skill_language_targets: z.array(z.string()).optional().describe('Programming languages this skill applies to (for create_skill)'),
    skill_framework_targets: z.array(z.string()).optional().describe('Frameworks this skill applies to (for create_skill)'),
    instruction_filename: z.string().optional().describe('Instruction filename (for get_instruction, create_instruction, delete_instruction, assign_instruction, unassign_instruction)'),
    instruction_applies_to: z.string().optional().describe('Glob pattern controlling which files/agents this instruction applies to (for create_instruction)'),
    instruction_content: z.string().optional().describe('Full instruction markdown content (for create_instruction)'),
    notes: z.string().optional().describe('Optional notes (for assign_skill, assign_instruction)'),
...
server.tool(
  ...
  'Terminal tool with GUI approval flow. Actions: run (execute command — auto-approves allowlisted, blocks destructive, shows GUI approval for others), spawn_cli_session (validated provider session spawn with provider/cwd/prompt/context payload), read_output (get output from a session), kill (terminate a session), get_allowlist (view allowlist), update_allowlist (manage allowlist patterns). SEQUENTIAL RULE: You MUST wait for each run response before calling run again. Concurrent run calls targeting the same session are automatically rerouted to a new terminal tab and will include a rate_limit_note in the response data.',
  {
    ...
    workspace_id: z.string().optional().describe('Workspace ID'),
    session_id: z.string().optional().describe('Session ID (for run/read_output/kill). For run, used when session_target=specific or as explicit override.'),
    session_target: z.enum(['selected', 'default', 'specific']).optional().describe('Run-session routing behavior: selected (default), default, or specific (requires session_id).'),
