...
5. **Session tracking**: `session_id` is minted by the extension during spawn prep and injected into the subagent's prompt.
6. **Subagents MUST include `_session_id`** in every tool call for session tracking (this is auto-injected in the enriched prompt).
7. **Stop directive compliance**: If a subagent receives a stop directive in a tool response, it should call `memory_agent(action: handoff)` with reason "User requested stop" then `memory_agent(action: complete)`.
...
- Spawned by hub agents using `memory_session` (action: prep) followed by native `runSubagent` (execute).
...
Hub agents should use `memory_session` (action: prep) to prepare context-rich spawn payloads, then execute with native `runSubagent`.
...
- `memory_session (action: prep)` is prep-only and MUST NOT be treated as execution.
